...
interface DiningWindowProps {
  onClose: () => void;
}

const DiningWindow: FC<DiningWindowProps> = ({ onClose }) => {
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40">
      <div className="flex w-full max-w-5xl flex-col overflow-hidden rounded-2xl bg-white shadow-xl">
        <header className="flex items-center justify-between bg-slate-700 px-6 py-3 text-white">
          <h2 className="text-[18px] leading-tight font-semibold">نقطة البيع</h2>
          <button
            type="button"
            onClick={onClose}
