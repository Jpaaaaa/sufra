...
const generate_report_pdf_1 = require("../../modules/reports/generate-report-pdf");
const generate_report_excel_1 = require("../../modules/reports/generate-report-excel");
class ReportsService {
    constructor(db, businessDayService) {
        this.db = db;
    ...
    async getDailySummary() {
        ...
        const orders = await this.db.all(`SELECT total, globalDiscount 
       FROM orders 
       WHERE created_at >= ? AND created_at < datetime('now') AND status = 'completed'`, [businessDayStart]);
        ...
        // Get table counts
        const tableRows = await this.db.all(`SELECT t.id, COUNT(o.id) as orderCount
       FROM tables t
       LEFT JOIN orders o ON t.id = o.table_id 
         AND o.status IN ('pending', 'printed')
       GROUP BY t.id`);
        const occupiedTables = tableRows.filter((t) => t.orderCount > 0).length;
        ...
        let printerStatus = 'success';
        try {
            const printerRow = await this.db.get(`SELECT COUNT(*) as count FROM printer_settings WHERE is_active = 1`);
    ...
    async generatePDF(dto) {
        try {
            ...
            if (!dto.data) {
                throw new exceptions_1.BadRequestException('Data is required for PDF generation');
            }
            ...
            return await (0, generate_report_pdf_1.generateReportPDF)(dto);
        }
        catch (error) {
    ...
    async generateExcel(dto) {
        return (0, generate_report_excel_1.generateReportExcel)(dto);
    }
    async getReportData(period, dateStr) {
        const date = new Date(dateStr);
        if (isNaN(date.getTime())) {
            throw new exceptions_1.BadRequestException('Invalid date format');
        }
        ...
        if (period === 'daily') {
            ...
            const businessDay = await this.db.get(`SELECT id, start_at, end_at, is_active 
         FROM business_days 
         ...
         LIMIT 1`, [dateStr]);
            ...
            if (period === 'weekly') {
                ...
                const businessDays = await this.db.all(`SELECT id, start_at, end_at, is_active 
           FROM business_days 
           ORDER BY start_at DESC 
           LIMIT 7`);
           ...
           ORDER BY start_at ASC`, [monthStartStr, monthEndStr]);
                if (!businessDays || businessDays.length === 0) {
                    // No business days found - return empty report
                    return {
                        summary: {
           ...
           FROM business_days 
           WHERE start_at >= ? AND start_at < ?
           ORDER BY start_at ASC`, [yearStartStr, yearEndStr]);
                if (!businessDays || businessDays.length === 0) {
                    // No business days found - return empty report
                    return {
                        summary: {
    ...
    async processWeeklyBusinessDays(businessDays) {
    ...
    async processMonthlyWeeks(businessDays, monthStart) {
        if (businessDays.length === 0) {
            return {
                summary: {
                    totalSales: 0,
        ...
        const weekPromises = weeks.map(async (week, weekIndex) => {
            ...
            let weekEarliestStart = null;
            let weekLatestEnd = null;
    ...
    async processYearlyMonths(businessDays, year) {
        if (businessDays.length === 0) {
            return {
                summary: {
                    totalSales: 0,
        ...
        const monthPromises = months.map(async (month, monthIndex) => {
            ...
            let monthEarliestStart = null;
            let monthLatestEnd = null;
            let hasActiveDay = false;
    ...
    async fetchOrdersAndProcess(period, startDate, endDate) {
        ...
        const orders = await this.db.all(`SELECT o.id, o.table_id, o.order_type, o.status, o.total, o.globalDiscount, 
              o.created_at, o.updated_at, o.customer_name, o.customer_phone, o.customer_location
       FROM orders o
       WHERE o.status = 'completed' 
         AND o.created_at >= ? 
         AND o.created_at < ${endDateCondition}`, [startDate]) || [];
    ...
    generateGraphData(period, startDate, endDate, orders) {
        const data = [];
        ...
        if (period === 'daily') {
            ...
            for (let i = 0; i < 24; i++) {
                ...
                const value = hourOrders.reduce((sum, o) => {
                    let discountAmount = 0;
                    if (o.globalDiscount) {
                        try {
            ...
            for (let i = 0; i < 7; i++) {
                ...
                // Calculate total sales including discounts for graph
                const value = dayOrders.reduce((sum, o) => {
                    let discountAmount = 0;
                    if (o.globalDiscount) {
                        try {
                            const globalDiscount = typeof o.globalDiscount === 'string'
            ...
            for (let i = 0; i < weeksInMonth; i++) {
                ...
                // Calculate total sales including discounts for graph
                const value = weekOrders.reduce((sum, o) => {
                    let discountAmount = 0;
                    if (o.globalDiscount) {
                        try {
                            const globalDiscount = typeof o.globalDiscount === 'string'
            ...
            for (let i = 0; i < 12; i++) {
                ...
                // Calculate total sales including discounts for graph
                const value = monthOrders.reduce((sum, o) => {
                    let discountAmount = 0;
                    if (o.globalDiscount) {
                        try {
                            const globalDiscount = typeof o.globalDiscount === 'string'
    ...
    generateDailyAggregates(orders, items, weekStartDate) {
    ...
    generateBusinessDayAggregates(businessDays, orders, items) {
        ...
        const sortedBusinessDays = [...businessDays].sort((a, b) => new Date(a.start_at).getTime() - new Date(b.start_at).getTime());
