...
export class OrdersService {
  ...
  /**
   * @deprecated This method is READ-ONLY. Use DineInOrdersService, PickupOrdersService, or DeliveryOrdersService instead.
   * This method will throw an error to prevent new orders from being created in the legacy orders table.
   */
  async create(data: {
    table_id: number;
    ...
    console.error('[ORDERS] ⛔ Use DineInOrdersService, PickupOrdersService, or DeliveryOrdersService instead');
    throw new BadRequestException(
      'Legacy orders table is read-only. Please use the new domain-specific order services: ' +
      'DineInOrdersService for dine-in orders, PickupOrdersService for pickup orders, ' +
      'or DeliveryOrdersService for delivery orders.'
    );
  ...
  async updateStatus(id: number, status: 'pending' | 'printed' | 'completed' | 'cancelled'): Promise<Order> {
  ...
  async updateOrderType(id: number, orderType: 'dine-in' | 'pickup' | 'delivery'): Promise<Order> {
    await this.db.run(
