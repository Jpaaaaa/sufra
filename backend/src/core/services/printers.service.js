...
class PrintersService {
    ...
    async getAllSettings() {
        const rows = await this.db.all('SELECT id, kitchen_id, printer_ip, printer_port, printer_type, is_active FROM printer_settings');
    ...
    async saveSetting(data) {
        ...
        const printer_type = data.kitchen_id !== null ? 'kitchen' : 'customer';
        ...
        if (row) {
            ...
            await this.db.run('UPDATE printer_settings SET printer_ip = ?, printer_port = ?, is_active = ?, kitchen_id = ?, printer_type = ? WHERE id = ?', [printer_ip, printer_port, is_active, data.kitchen_id, printer_type, row.id]);
            return {
                id: row.id,
            ...
            // Insert new
            await this.db.run('INSERT INTO printer_settings (kitchen_id, printer_ip, printer_port, printer_type, is_active) VALUES (?, ?, ?, ?, ?)', [data.kitchen_id, printer_ip, printer_port, printer_type, is_active]);
    ...
    formatOrderForPrint(kitchenName, orderNumber, items, tableName) {
        const LF = '\n';
        let output = '';
    ...
    formatReceiptForPrint(tableNumber, hallName, orders, total, subtotal, globalDiscount) {
        const LF = '\n';
        let output = '';
