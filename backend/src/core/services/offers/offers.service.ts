...
export class OffersService {
  ...
  async createDailyDeal(data: { product_id: number; special_price: number; date: string }): Promise<DailyDeal> {
    ...
    await this.db.run(
      'INSERT INTO daily_deals (product_id, special_price, date) VALUES (?, ?, ?)',
  ...
  // ========== Combos ==========
  async createCombo(data: { combo_name: string; combo_price: number; product_ids: number[] }): Promise<Combo> {
    ...
    await this.db.run(
      'INSERT INTO combos (combo_name, combo_price) VALUES (?, ?)',
      [data.combo_name, data.combo_price],
    );
  ...
  async updateCombo(id: number, data: { combo_name?: string; combo_price?: number; product_ids?: number[]; is_active?: number }): Promise<Combo> {
  ...
  async createHappyHour(data: {
    ...
    await this.db.run(
      'INSERT INTO happy_hour (product_id, happy_hour_price, time_start, time_end) VALUES (?, ?, ?, ?)',
      [data.product_id, data.happy_hour_price, data.time_start, data.time_end],
    );
    const id = await this.db.getLastInsertRowId();
    return {
      id: id,
  ...
  async getHappyHour(id: number): Promise<HappyHour> {
    ...
    return row as HappyHour;
  }

  async updateHappyHour(
    id: number,
    data: { happy_hour_price?: number; time_start?: string; time_end?: string; is_active?: number },
  ): Promise<HappyHour> {
  ...
  async getActiveHappyHourPrice(product_id: number): Promise<number | null> {
    ...
    const row = await this.db.get(
      `SELECT happy_hour_price 
       FROM happy_hour 
       WHERE product_id = ? 
       AND is_active = 1 
       AND time_start <= ? 
       AND time_end >= ?`,
      [product_id, currentTime, currentTime],
    );
